...
class ChatServer:
    def __init__(self, host='0.0.0.0', port=5555):  # Изменено на 0.0.0.0
        self.host = host
        self.port = port
    ...
    def start(self):
        try:
            ...
            print(f"📍 Локальный адрес: {local_ip}:{self.port}")
            print(f"📍 Адрес для подключения: {self.host}:{self.port}")
...
if __name__ == "__main__":
    server = ChatServer(host='0.0.0.0', port=5555)  # Или ваш конкретный IP
    try:
        server.start()
